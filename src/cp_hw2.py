...
def writeEXR_1D(name, data, cols, rows):

    exr = OpenEXR.OutputFile(name, OpenEXR.Header(cols, rows))

    R = (data[0::3]).astype(np.float32).tobytes()
    G = (data[1::3]).astype(np.float32).tobytes()
    B = (data[2::3]).astype(np.float32).tobytes()
